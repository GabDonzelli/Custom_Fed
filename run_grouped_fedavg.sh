...
#SBATCH --job-name=grouped-fedavg
#SBATCH --partition=sra
...
# Diretorio temporario do job dentro do meu espaco pfcalcul
TMP_BASE="${PROJECT_DIR}/tmp"
...
JOB_TMP="${SLURM_TMPDIR:-${TMP_BASE}/custom_fed_${USER}_${JOB_ID}}"
...
flwr run . --stream \
    --federation-config="num-supernodes=100 client-resources-num-cpus=2 client-resources-num-gpus=0 init-args-num-cpus=${ALLOCATED_CPUS} init-args-num-gpus=0"
